...
//BEGIN FUNCTIONS
//BEGIN UPDATE
void update(Uint64 acc, Uint64 dt)
{
	if (log_dt){
		Uint64 ms = dt/1000/1000;
		Uint64 rest = dt - (ms*1000*1000);
		// SDL_Log("DT:	%lu", ms);
		// SDL_Log("Rest:	%lu", rest);
		SDL_Log("DT in ms:	%lu.%lu", ms,rest);
	}
	...
	SDL_Log("G: %f",g);
	SDL_Log("dt: %lu",dt);
	// float f_ms = dt/1000/1000;
	float f_s = (float)dt / (float)frequency;
	SDL_Log("dt: %f", f_s);
	...
	// box.data.frac.pos.y += f_s * v;

/*
	if (Altitude > 0){
		Altitude 		= distance_m - 0.5 * g * acc/frequency * acc/frequency;
...
*/
	...
	if (box.data.frac.pos.y >= (float)wh - box.data.frac.size.y){
		//box.data.frac.pos.y = (float)wh - box.data.frac.size.y;
...
void update_desc(Uint64 time_acc, float z)
{
	float time = (float)time_acc/(float)frequency;
	char buffer[10];
