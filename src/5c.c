...
//END WINDOW
long double currentTime = 0.0f;
long double time_acc = 0.0f;
...
while(running){
	...
	long double newTime   		=  time_();
	long double deltaTime 		=  newTime - currentTime;
	currentTime     		=  newTime;
	time_acc    			+= deltaTime;
	counter				+= deltaTime;
	update				   (deltaTime);
