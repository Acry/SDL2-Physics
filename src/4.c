 ...
 * Derivatives 1
 * 
 * Due to the constant accelleration caused by gravity,
 * the result in Gravity 1
 * https://github.com/Acry/SDL2-Physics/blob/master/src/3.c
 * is far from precise.
 * When the container hits the ground the velocity is so high
 * that the point in time of ground impact is missed.
 * 
 * I try to fix that now utilizing derivatives.
 * Going to try to interploate the velocity and position in a finer grain.
 * 
 * 
 * Altitude in meter
 * Velocity in m/s
 * Accelleration in m/s^2
 * 
 * spring constant, k,
 */
